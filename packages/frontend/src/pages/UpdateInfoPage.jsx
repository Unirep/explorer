...
export default observer(() => {
  ...
  const info = unirep.descriptionsByAttesterId.get(attesterId) || {icon: '', name: '', description: '', url: ''}
  const [icon, setIcon] = useState(info.icon)
  const [name, setName] = useState(info.name)
  const [description, setDescription] = useState(info.description)
  const [url, setUrl] = useState(info.url)
  const infoHash = utils.keccak256(utils.toUtf8Bytes(JSON.stringify([icon, name, description, url])))
    ...
    <div className="content">
      ...
      <div className="info-page-container">
        <div>
          ...
          <div className='info-p'>Enable users to discover your application.</div>
          <div className='signature-container'>
            <h5 style={{marginBottom: '2rem'}}>Upon updating the project information, it's mandatory to provide a signature. It is crucial to be prepared to substantiate that you are indeed the attester contract's owner.</h5>
            <div className='info-p' style={{fontWeight: '600'}}>Hash of attester info:</div>
            ...
            <Button 
              ...
              onClick={async () => {
                const response = await unirep.updateAttesterDescription(
                  attesterId, icon, url, name, description
                )
            ...
            {responseMessage === 'info updated!' ?
              <h5 style={{textAlign: 'center'}}>
                ...
                <Link to={`/attester/${id}`}> <span style={{textDecoration: 'underline'}}>here</span></Link>
              </h5>    
            : null }
          </div>
        </div>
        ...
        <div className='form-container'>
          ...
          <div className='form-section'>
            ...
            <h5>Tell us about your attester. What does it do? What problem does it solve?</h5>
            <textarea
              placeholder='ZKP...'
              value={description ?? ''}
              onChange={(event) => {
